  ...
  <div class="middle">
	...
	<div class="content">
        <table width="100%">
          <tr>
           ...
           <div class="product-tab">
			...
			<?php } } } else {  ?>
              <input type="text" name="name" value="<?php echo $name_fb; ?>" />
              <?php if ($error_name) { ?>
              <span class="error"><?php echo $error_name; ?></span>
              <?php } ?><br />
             <?php echo $entry_city ?><br />
			 <input type="text" name="city" value="<?php echo $hometown_fb; ?>" style="margin-bottom: 6px;" />
			 <br />
			<?php } ?>
			<!-- Popup Example -->
			<?php if (isset($_COOKIE['Social'])) { ?>
			...
			<?php foreach ($infos as $info) { ?>
			<?php if ($_COOKIE["Social"] == $info['uid']) { ?>
			<img src="<?php echo $info['image']; ?>" style="width:40px;height:40px;" />
			Logged by <?php echo $info['user_type']; ?>
			<?php } } } ?>
			<br>
			
			
			<?php if (isset($_COOKIE['Social'])) { ?><?php } else { ?>
			 <?php echo $entry_photo; ?><br />
			 <div id="drop" style="float: left;"><a href="//smart.md/upload_image/" class="iframe" id="upload_image"><?php echo $upload_photo; ?></a></div>
			<?php } ?>
      ...
      <div class="buttons">
		...
		<td align="right" style="float: right;margin-right: 80px;"><a  onclick="$('#testimonial').submit();" class="button but_yellow"><span><?php echo $add_button; ?></span></a>
		</td>
          </tr>
        </table>
		<!--
